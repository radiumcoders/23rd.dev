---
title: Stretchy Footer
description: Dia-style rubber overscroll — pull past the bottom and a rainbow aurora stretches up, then springs back when you let go.
---

import { StretchyFooterDemo } from "@/registry/stretchy-footer/stretchy-footer-demo"

Not a scroll-progress footer reveal — an **overflow rubber-band**. In the preview, scroll past the end (or hit Show effect): the page rolls up from the bottom and the aurora fills the stretch. Swap palettes, blur, glow, and snap below.

<StretchyFooterDemo />

## Installation

<CliCommand item="stretchy-footer" />

Or install straight from the public GitHub source registry:

<CliCommand item="stretchy-footer" github />

## Usage

Use it as the scroll viewport. Put your page (and a normal footer block) in children. Wheel / touch past the bottom stretches the aurora; idle or release snaps it back. Honors `prefers-reduced-motion` (no stretch).

<FrameworkCode>
  <FrameworkReact>
```tsx
"use client"

import { StretchyFooter } from "@/components/ui/stretchy-footer"

export function Page() {
  return (
    <StretchyFooter className="h-svh bg-background">
      <main className="min-h-[120%] px-8 py-24 text-foreground">
        {/* …page content… */}
      </main>
      <footer className="px-8 pb-10 text-center text-muted-foreground">
        23rd — opinionated UI for shippers
      </footer>
    </StretchyFooter>
  )
}
```
  </FrameworkReact>
  <FrameworkSvelte>
```svelte
<script>
  import StretchyFooter from "$lib/components/ui/stretchy-footer.svelte"
</script>

<StretchyFooter class="h-svh bg-background">
  <main class="min-h-[120%] px-8 py-24 text-foreground">
    <!-- page content -->
  </main>
  <footer class="px-8 pb-10 text-center text-muted-foreground">
    23rd — opinionated UI for shippers
  </footer>
</StretchyFooter>
```
  </FrameworkSvelte>
</FrameworkCode>

On a full document (window scroll), pass `windowScroll` and mark the page root with `data-stretchy-page` so the content rolls up into the aurora:

<FrameworkCode>
  <FrameworkReact>
```tsx
"use client"

import { StretchyFooter } from "@/components/ui/stretchy-footer"

export function DocsChrome() {
  return (
    <>
      <div data-stretchy-page>{/* page content */}</div>
      <StretchyFooter windowScroll />
    </>
  )
}
```
  </FrameworkReact>
  <FrameworkSvelte>
```svelte
<script>
  import StretchyFooter from "$lib/components/ui/stretchy-footer.svelte"
</script>

<div data-stretchy-page><!-- page content --></div>
<StretchyFooter windowScroll />
```
  </FrameworkSvelte>
</FrameworkCode>

## Props

| Prop | Type | Default |
| --- | --- | --- |
| `children` | `ReactNode` / snippet | — |
| `scrollRef` | `RefObject<HTMLElement \| null>` | React — self as scroller |
| `scrollEl` | `HTMLElement` | Svelte — self as scroller |
| `windowScroll` | `boolean` | `false` |
| `contentSelector` | `string` | `[data-stretchy-page]` |
| `maxStretch` | `number` | `280` |
| `colors` | `string[]` | spectrum red → pink |
| `stiffness` | `number` | `380` |
| `damping` | `number` | `32` |
| `columns` | `number` | `48` |
| `blur` | `number` | `14` |
| `glow` | `number` | `0.22` |
| `label` | `string` | `Stretchy overflow` |
| `className` | `string` | React only |
| `class` | `string` | Svelte only |
